...
typedef struct
{
    ...
    void(*fill)(uint16_t,uint16_t,uint16_t,uint16_t,uint32_t);		///void fill(u16 sx,u16 sy,u16 ex,u16 ey,u32 color) 单色填充函数
    void(*draw_hline)(uint16_t,uint16_t,uint16_t,uint16_t);		//void draw_hline(u16 x0,u16 y0,u16 len,u16 color)  画水平线函数
    void(*fillcolor)(uint16_t,uint16_t,uint16_t,uint16_t,uint16_t*);	//void piclib_fill_color(u16 x,u16 y,u16 width,u16 height,u16 *color) 颜色填充
}_pic_phy;
