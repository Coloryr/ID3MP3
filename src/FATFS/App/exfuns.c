...
///////////////////////////////公共文件区,使用malloc的时候////////////////////////////////////////////
ramfast FATFS *fs[_VOLUMES];//逻辑磁盘工作区.
ramfast FIL *file;            //文件1
ramfast FIL *ftemp;            //文件2.
ramfast UINT br, bw;            //读写变量
ramfast FILINFO fileinfo;    //文件信息
ramfast DIR dir;            //目录

ramfast uint8_t *fatbuf;            //SD卡数据缓存区
///////////////////////////////////////////////////////////////////////////////////////
//为exfuns申请内存
//返回值:0,成功
//1,失败
...
exf_copy(uint8_t(*fcpymsg)(uint8_t *pname, uint8_t pct, uint8_t mode), uint8_t *psrc, uint8_t *pdst, uint32_t totsize,
         uint32_t cpdsize, uint8_t fwmode) {
    uint8_t res;
    uint16_t br = 0;
    uint16_t bw = 0;
    FIL *fsrc = 0;
...
#define MAX_PATHNAME_DEPTH    512+1    //最大目标文件路径+文件名深度
    ...
    if (res == 0) {
        pathname = (uint8_t *) malloc(MAX_PATHNAME_DEPTH);
