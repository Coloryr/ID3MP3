...
{
	u8 i;
	myfree(iclip);			//释放iclip申请到的内存
	myfree(jpg_buffer);		//释放jpg_buffer申请到的内存
	for(i=0;i<4;i++)
	{
		if(i<3)myfree(qt_table[i]);	//释放内存	
		myfree(code_pos_table[i]); 	//释放内存	
		myfree(code_len_table[i]); 	//释放内存	
		myfree(huf_max_value[i]); 	//释放内存	
		myfree(huf_min_value[i]); 	//释放内存	
		myfree(code_value_table[i]);	//释放内存	  	 
	}
	myfree(MCUBuffer);		//释放内存	 
	myfree(QtZzMCUBuffer);	//释放内存	 
	myfree(BlockBuffer);		//释放内存	 
	myfree(Y);				//释放内存	 
	myfree(U);				//释放内存	 
	myfree(V);				//释放内存	 
}
